...
CGOpenMPRuntime::createRuntimeFunction(unsigned Function) {
  ...
  case OMPRTL__kmpc_flush: {
    // Build void __kmpc_(ident_t *loc);
    llvm::Type *TypeParams[] = {getIdentTyPointerTy()};
    llvm::FunctionType *FnTy =
        llvm::FunctionType::get(CGM.VoidTy, TypeParams, /*isVarArg*/ false);
  ...
  case OMPRTL__kmpc_allocate: {
    // Build void __kmpc_flush(ident_t *loc);
    llvm::Type *TypeParams[] = {getIdentTyPointerTy()};
    llvm::FunctionType *FnTy =
        llvm::FunctionType::get(CGM.VoidPtrTy, TypeParams, /*isVarArg*/ false);
    RTLFn = CGM.CreateRuntimeFunction(FnTy, "malloc",llvm::AttributeSet(400));
    //RTLFn = malloc(400);
    break;
  }
...
void CGOpenMPRuntime::emitAllocate(CodeGenFunction &CGF, ArrayRef<const Expr *>,
                                SourceLocation Loc) {
  if (!CGF.HaveInsertPoint())
    return;
  // Build call void __kmpc_flush(ident_t *loc)
  CGF.EmitRuntimeCall(createRuntimeFunction(OMPRTL__kmpc_allocate),
                      emitUpdateLocation(CGF, Loc));
}
